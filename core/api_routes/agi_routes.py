...
async def chat(req: ChatRequest, current_user=Depends(get_current_user)):
    ...
    # 音频输入
    if req.audio:
        result = agi_coordinator.execute_task_with_audio(query, req.audio, user_id=current_user["id"])
        return {
    ...
    # 视频输入
    if req.video:
        result = agi_coordinator.execute_task_with_video(query, req.video, user_id=current_user["id"])
        return {
            "answer": result["message"],
            "source": "video",
            "module_id": None,
    ...
    # 图片输入
    if req.image:
        result = agi_coordinator.execute_task_with_image(query, req.image, user_id=current_user["id"])
        if result["success"]:
            return {
